...
def preprocess_text(text):
    ...
    # تم التعديل إلى \w ليدعم اللغة العربية والأرقام بدون مسحها
    text = re.sub(r'[^\w\s\-\.]', ' ', text)
    return re.sub(r'\s+', ' ', text).strip()


def create_search_text(row):
    # تم دمج البحث في title و name لتفادي الأخطاء
...
def match_products(source_df, target_df, top_k=5):
    ...
    # جلب البيانات بطريقة آمنة تمنع الأخطاء لو كان أحد الحقول مفقوداً
    target_data = []
    ...
    for _, row in source_df.iterrows():
        # دعم جلب الاسم من title أو name
        ...
        for score, idx in score_pairs[:top_k]:
            ...
            results.append({
                ...
                'target_price': float(target['price']) if target['price'] else 0.0,
...
def load_retailer_products():
    ...
    try:
        if os.path.exists(csv_path):
            ...
            # توحيد أسماء الأعمدة لتتطابق مع البيانات الجديدة المسحوبة (title -> name)
            rename_map = {'title': 'name', 'source': 'retailer', 'link': 'url'}
            df = df.rename(columns=rename_map)

            # التأكد من وجود الأعمدة الأساسية حتى لا يتوقف الكود عن العمل
